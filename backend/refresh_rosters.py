    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            if existing_batch_id:
                ...
                async with tracker:
                    await _run_refresh_logic(
                        db, tracker, client, dry_run, backfill_history,
                        positions, max_backfill_weeks, skip_confirmation,
                        start_time
                    )
            else:
...
async def _run_refresh_logic(
    db, tracker, client, dry_run, backfill_history,
    positions, max_backfill_weeks, skip_confirmation, start_time
):
    """Extracted refresh logic to reuse for both new and existing batches"""
