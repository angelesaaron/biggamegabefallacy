...
with tab_player:
    ...
    if not selected_team_row.empty:
        ...
        if not selected_player_row.empty:
            ...
            st.dataframe(playerExperienceDF)
            if not playerExperienceDF.empty:
                ...
                if not gameLog.empty:
                    ...
                    # DISPLAY GAME LOG
                    gameLogTransformed = gameLog[['seasonYr', 'week', 'receptions', 'receivingTargets' ,'receivingYards', 'receivingTouchdowns']]
                    gameLogDisplay = gameLogTransformed.rename(columns={'seasonYr':'Season',
                                                                        'week':'Week',
                    ...
                    with st.expander("View Game Log", expanded=False):
                        ...
                        st.dataframe(gameLogDisplay.drop_duplicates())
                        st.dataframe(gameData)
